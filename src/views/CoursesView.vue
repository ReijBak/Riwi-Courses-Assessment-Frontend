  ...
  <AppLayout>
    <div class="p-4 md:p-8 w-full">
      ...
      <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 md:mb-8">
        <div>
          <h1 class="text-2xl md:text-3xl font-bold text-slate-800">📚 Courses</h1>
          <p class="text-gray-500 mt-1 text-sm md:text-base">Manage your courses and lessons</p>
        </div>
        <button
      ...
      <div class="flex flex-col sm:flex-row gap-3 md:gap-4 mb-6 md:mb-8">
        ...
        <select
          ...
          class="w-full sm:w-auto px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-indigo-500 outline-none bg-white"
        >
      ...
      <!-- Course List -->
      <div v-else-if="courseStore.courses.length > 0" class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4 md:gap-6">
        <CourseCard
      ...
      <!-- Pagination -->
      <div v-if="courseStore.totalPages > 1" class="flex flex-col sm:flex-row justify-center items-center gap-4 sm:gap-6 mt-8 md:mt-10">
        <button
          :disabled="courseStore.currentPage === 1"
          @click="changePage(courseStore.currentPage - 1)"
          class="w-full sm:w-auto px-5 py-2.5 border-2 border-indigo-500 text-indigo-500 rounded-lg font-medium hover:bg-indigo-500 hover:text-white disabled:opacity-50 disabled:cursor-not-allowed transition-all"
        >
          ← Previous
        </button>
        <span class="text-gray-600 text-sm md:text-base text-center">
